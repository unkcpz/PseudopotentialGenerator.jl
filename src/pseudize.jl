...
function pseudize_TM(
    ...
    function fixpoint_tm(c2_x, params)
        ...
        rhs[4] =
            2 * vprc + 2 * (nl.l + 1) / rc^2 * rhs[2] - 2 * (nl.l + 1) / rc * rhs[3] -
            2 * rhs[2] * rhs[3]
        rhs[5] =
            2 * vpprc - 4 * (nl.l + 1) / rc^3 * rhs[2] + 4 * (nl.l + 1) / rc^2 * rhs[3] -
            2 * (nl.l + 1) / rc * rhs[4] - 2 * rhs[3]^2 - 2 * rhs[2] * rhs[4]

        M_lhs = [
            1 rc^6 rc^8 rc^10 rc^12
