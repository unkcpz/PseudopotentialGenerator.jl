function solve_radial_eigenproblem(n, l, Z, V, mesh; tol=1e-9, max_iter=200, Emax = 0.0, Emin = -8000.0, E_ini = -1000.0, rel = 0, perturb = false)::Tuple{Float64, Vector{Float64}, Vector{Float64}}
    N = length(mesh.r)
    P = zeros(Float64, N)
    Q = zeros(Float64, N)
    Vs = V.(mesh.r)
